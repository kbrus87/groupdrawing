...
export const evaluateDrawing = async (
  ...
  const ai = new GoogleGenAI({ apiKey: apiKey || import.meta.env.REACT_APP_AI_API_KEY });
  let referenceBase64 = "";
  try {
  ...
  const isEncouraging = ["Boy", "Girl", "LittlePerson"].includes(
    participantType
  );
  let specificInstruction = isEncouraging
    ? "El dibujante es un niño. Sé amable, positivo y constructivo. Puntuación mínima 50."
    : "El dibujante es un adulto. Puedes ser sarcástico (roast) pero termina con un consejo técnico útil o un rasgo positivo del dibujo.";
  ...
  try {
    ...
    if (error.toString().includes("RESOURCE_EXHAUSTED")) {
      alert("Ha agotado su cuota de Gemini. Utilice otra ApiKey");
    }
    return {
      score: 70,
      feedback: "Ocurrio un error, pero te aprobamos igual!.",
    };
  }
};
...
const getRandomUnsplashId = (category: string): string => {
  const pools: Record<string, string[]> = {
    Dynamic: [
      "1536922246237-34c205439d58",
      "1508700929628-666bc8bd84ea",
      "1541271696563-3be2f555fc4e",
      "1493106640158-4788174bc97c",
      "1471286174293-9b7890029db9",
      "1516062423079-7c1bd7c52f35",
    ],
    Athletic: [
      "1517836357463-d25dfeac3438",
      "1526506118085-60ce8714f8c5",
      "1434596922112-19c563067271",
      "1483721310020-0ac331adad8e",
    ...
    Sitting: [
      ...
      "1485811661307-aaca90250663",
    ],
    Standing: [
      "1501196354995-cbb51c65aaea",
      "1524504388940-b1c1722653e1",
      "1499952127939-9bbf5af6c51c",
      "1539571696357-5a69c17a67c6",
      "1515161311255-58190cabc9c0",
    ],
  };
